...
st.title(f"Ajouter un prompt pour Manche 2, {username}")
st.image('static/assets/manche2.webp', width=600)
prompt = st.text_area(f"Essaie de reproduire cette image, {username}")

if st.button("Ajouter"):
    if prompt:
        image_url = generate_image_openai(prompt)
        prompts_collection.insert_one({"username": username, "prompt": prompt, "image_url": image_url, "round": 2})
